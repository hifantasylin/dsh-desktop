...
const COPY: Record<DesktopLocale, DesktopRecoveryCopy> = {
  en: {
    title: 'Rouba DSH Recovery Assistant',
    fallbackBody: 'The recovery information could not be read. Quit and start Rouba DSH again.',
    ...
    configurationFilesBody: 'View or edit the current Profile and shared Harness-home configuration. Restart Rouba DSH after making changes.',
    ...
    desktopVersion: 'Rouba DSH version',
    pluginCount: 'Plugins',
    ...
    confirmRollbackBody: capturedAt => `This immediately restores the current Profile plus the checkpointed settings.yaml and Harness-home patch captured at ${capturedAt}. After restarting, Rouba DSH will use the rolled-back configuration.`,
    confirmRollbackAction: 'Roll Back',
    uninstalledSuccess: 'The plugin was removed from the current Profile. Restart Rouba DSH to use the updated plugin configuration.',
    rollbackSuccess: slotId => `Rolled back to ${slotId}. Restart Rouba DSH to use this configuration; the first healthy start after rollback will preserve all three existing slots.`,
    profileSelectedSuccess: 'This Profile is now selected. Restart Rouba DSH to use it.',
  ...
  zh: {
    title: 'Rouba DSH 恢复助手',
    fallbackBody: '无法读取恢复信息。请退出并重新启动 Rouba DSH。',
    reason: '进入恢复模式的原因',
    ...
    configurationFilesBody: '查看或编辑当前 Profile 与 DSH home 的共享配置。修改后需要重新启动 Rouba DSH。',
    ...
    confirmRollbackBody: capturedAt => `将立即恢复 ${capturedAt} 创建的 Checkpoint 中的当前 Profile、settings.yaml 与 DSH home 补丁；重启后，Rouba DSH 将使用回滚后的配置。`,
    confirmRollbackAction: '回滚',
    uninstalledSuccess: '插件已从当前 Profile 中卸载。请重启 Rouba DSH 以使用更新后的插件配置。',
    rollbackSuccess: slotId => `已回滚到${slotId}。请重启 Rouba DSH 以使用该配置；回滚后的第一次健康启动会保留现有三个槽位。`,
    profileSelectedSuccess: '已设为当前 Profile。请重启 Rouba DSH 以使用该 Profile。',
