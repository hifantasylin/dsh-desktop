 ...
 */
export function arkStatusMeaning(status) {
    switch (status) {
...
export function arkHeaders(apiKey) {
    return {
...
function wrapFetchError(stage, error) {
    if (isAbort(error) || error instanceof ArkHttpError || error instanceof ArkNetworkError)
        return error;
 ...
 */
export async function probeResult(url, signal) {
    let response;
    try {
