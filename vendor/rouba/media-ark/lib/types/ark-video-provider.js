...
import { getJson, postJson, probeResult, ArkHttpError, ArkNetworkError, arkStatusMeaning, isNetworkError } from "./http.js";
...
const CONSERVATIVE_CAPS = { minDuration: 2, maxDuration: 12, maxImageUrls: 2, maxVideoUrls: 0, maxAudioUrls: 0 };
...
function capsForModel(model) {
    for (const entry of CAPS_BY_GENERATION) {
        if (entry.generation.test(model))
            return entry.caps;
    }
...
export class ArkVideoProvider extends VideoProvider {
    ...
    async finalize(handle, signal, onProgress) {
        ...
        if (probe === undefined || probe.status < 200 || probe.status >= 300) {
            throw new ArkHttpError(`Ark task ${handle.taskId} result URL is unreachable [${probe?.status ?? 'no-body'}]`, probe?.status);
        }
...
//# sourceMappingURL=ark-video-provider.js.map