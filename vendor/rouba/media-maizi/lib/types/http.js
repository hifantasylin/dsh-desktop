 ...
 */
export function httpStatusMeaning(status) {
    switch (status) {
...
export function statusTag(error) {
...
function wrapFetchError(stage, error) {
    if (isAbort(error) || error instanceof MaiziHttpError || error instanceof MaiziNetworkError)
        return error;
 ...
 * immediately.
 */
export async function postJson(url, apiKey, body, signal) {
    let lastError;
 ...
 */
export async function getJson(url, apiKey, signal) {
    let lastError;
    for (let attempt = 0; attempt < RETRY_ATTEMPTS; attempt++) {
        const isLast = attempt === RETRY_ATTEMPTS - 1;
        try {
...
async function downloadOnce(url, signal, options) {
    ...
    for (;;) {
        ...
        if (outcome === 'timeout') {
            ...
            throw new MaiziHttpError(`download inactivity timeout after ${options.inactivityTimeoutMs}ms (${received}/${totalBytes ?? '?'} bytes)`);
        }
 ...
 * a corrupt file.
 */
export async function downloadBytes(url, signal, options) {
    const resolved = {
 ...
 */
export async function streamBytes(url, signal, options) {
    let response;
    try {
        response = await fetch(url, {
            method: 'GET',
