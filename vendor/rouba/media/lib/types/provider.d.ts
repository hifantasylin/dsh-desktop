...
export declare abstract class ImageProvider {
    ...
    abstract generate(input: ImageGenerateInput, signal?: AbortSignal, onProgress?: (progress: MediaProgress) => void): Promise<ImageGenerationResult>;
...
export declare abstract class VideoProvider {
    abstract readonly provider: string;
    abstract readonly defaultModel: string;
    ...
    abstract finalize(handle: VideoTaskHandle, signal?: AbortSignal, onProgress?: (progress: MediaProgress) => void): Promise<VideoGenerationResult>;
    /** Bounds this provider enforces for the given model (its default when omitted). */
