...
import { cachedMediaBytes, registerLocalMedia } from "../media-cache.js";
...
const CATEGORIES = ['upload', 'character', 'scene', 'prop', 'keyframe', 'video', 'storyboard', 'cover', 'meta'];
...
export function registerMediaAssetSave(ctx) {
    const disposers = [];
        ...
        async execute(args, exec) {
            ...
            const jobId = ctx.jobs.start({
                ...
                run: () => {
                    ...
                    return { cancel: (reason) => { ac.abort(reason); }, done };
                },
            });
            return { kind: 'background', jobId, target: filePath };
        },
        presentCall(args) {
            return {
                card: 'generic',
...
async function fetchBytes(ctx, source, signal) {
    switch (source.kind) {
        ...
        default: {
            ...
            throw new Error(`media_asset_save: unsupported source kind ${String(source.kind)}`);
        }
    }
}
/** Extract a public https URL from a reference value, if present. */
export function extractPublicUrl(reference) {
...
export async function appendIndex(assetsDir, entry) {
...
//# sourceMappingURL=media-asset-save.js.map