 ...
 */
import { DEFAULT_PROVIDER_ID, MEDIA_CATEGORY_DEFAULT_ADAPTERS, MEDIA_CATEGORY_DEFAULTS, ROUBAAI_SETTINGS_NS, RoubaaiMediaSettingsSchema, } from "./config.js";
import { adapterCatalog, probeViaAdapter } from "./adapters.js";
/**
...
export async function testConnection(baseUrl, apiKey) {
    ...
    try {
        ...
        if (response.ok)
            return { ok: true, message: `连接成功（HTTP ${String(response.status)}）` };
 ...
 * @returns whether the endpoint answered favorably, plus the human reason.
 */
export async function testMusicConnection(baseUrl, apiKey) {
    if (apiKey.trim() === '')
        return { ok: false, message: '未填写 API Key' };
    const base = baseUrl.trim().replace(/\/+$/, '');
    if (base === '')
...
export function apply(ctx) {
    ...
    const viewOf = () => {
        ...
        if (descriptor === undefined)
            return { value: undefined, revision: undefined, secrets: [] };
        ...
        'settings.update': async (payload) => {
            ...
            try {
                ...
                if (code === CONFLICT_CODE) {
                    const failure = new Error('配置已被其他端修改，请刷新后重试');
                    failure.status = 409;
                    throw failure;
                }
                throw error;
    ...
    void migrateLegacySettings(ctx).catch(() => { });
}
